...
from VDRtest.util.image_pool import ImagePool
import torch.nn as nn
import cv2
from VDRtest.models.base_model import BaseModel
from VDRtest.models import networks
import torch.nn.functional as F
from VDRtest import cloth_filter
import matlab.engine
from VDRtest.models import MLS_demo
...
class Pix2PixHDModel(BaseModel):
    ...
    def multi_scale_blend(self,fake_img,fake_c,mask,number=4):
        ...
        out+=(1-mask)*fake_img
        return out
    def forward(self, label, pre_clothes_mask, img_fore, clothes_mask, clothes, all_clothes_label, real_image, pose,grid,mask_fore, cloth_before):
        # Encode Inputs
        ...
        cm_in_person = torch.FloatTensor((label.cpu().numpy() == 4).astype(np.float)).cuda()
        ...
        clothes_before = cloth_before.float().cuda().squeeze()
        ...
        erosion = cv2.dilate(warped_mask_temp, kernel, iterations=3)
        ...
        Warped_mask_tensor = transform1(Warped_mask).cuda()
        Warped_mask_tensor = Warped_mask_tensor.unsqueeze(0)
        Warped_mask_tensor = self.sigmoid(Warped_mask_tensor) + fake_cl_dis
        Warped_Mask = self.ReLu(Warped_mask_tensor - 1.5)*8
        supplement_mask = fake_cl_dis - self.sigmoid(Warped_mask_tensor)
        supplement_mask = self.ReLu(supplement_mask - 0.15) * 1000 - 6
        supplement_mask = self.sigmoid(supplement_mask)
        supplement_mask = torch.FloatTensor((supplement_mask.cpu().numpy() == 1).astype(np.float)).cuda()

        Warped_tensor = Warped_tensor_temp *Warped_Mask
        ...
        dis_label_F = dis_label
        dis_label_F[:, 11:12] = dis_label_F[:, 11:12] + supplement_mask
        G_in_1 = torch.cat([img_hole_hand, dis_label_F, Warped_tensor, skin_color, self.gen_noise(shape)], 1)
        fake_image_1 = self.G.refine(G_in_1.detach())
        fake_image_1 = self.tanh(fake_image_1)


        return [fake_image_1,clothes, arm_label,fake_cl, real_image]
